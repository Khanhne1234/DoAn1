    ...
    if (!empty($checkToken)) {
        if (isPost()) {
            ...
            if (empty(trim($filter['password']))) {
                $errors['password']['required'] = 'Mật khẩu bắt buột phải nhập';
            } else {
            ...
            if (empty($errors)) {
                ...
                if ($UpdateStatus) {
                    ...
                    $content .= 'Nếu không phải bạn thao tác đổi mật khẩu hãy liên hệ ngay với admin. <br>';
                    $content .= 'Cảm ơn bạn đã ủng hộ khóa học này!!';
                    //gửi mail
                    sendMail($emailTo, $subject, $content);
                    setSessionFlash('msg', 'Đổi mật khẩu thành công, bạn có thể đăng nhập ngay bây giờ.');
                    setSessionFlash('msg_type', 'success');
                } else {
                    setSessionFlash('msg', 'Đã có lôi xảy ra, vui lòng thử lại.');
